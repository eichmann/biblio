...
<table>
	<tr>
		<td>
			<c:if test="${limit > 0 && offset > 0}"><a href="TactileList.jsp?limit=${limit}&offset=${offset - limit}">&lt;</a></c:if>
		</td>
		<td>
			<c:if test="${limit > 0}"><a href="TactileList.jsp?limit=${limit}&offset=${offset + limit}">&gt;</a></c:if>
		</td>
	</tr>
</table>

<ul>
<biblio:foreachTactile limitCriteria="${limit}" offsetCriteria="${offset}" >
   <biblio:Tactile>
